...
Webサイトではクローリングを制御するためにrobots.txtが置かれている。
...
例：[無料天気予報API（OpenWeatherMap）](https://openweathermap.org/api)を使う
...
```
# 以下の情報についてプログラミングや、設定ファイルを設定する
# 1.どのサイトをクローリングするか
# 2.いつ、どのぐらいの時間感覚でクローリングするか
```
